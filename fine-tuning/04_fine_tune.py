...
Fine-tunes the all-MiniLM-L6-v2 embedding model using MultipleNegativesRankingLoss
on synthetic (query, product) pairs. Uses in-batch negatives to learn better
query-product alignment in the embedding space.
"""
...
from config import (
    BASE_MODEL, TRAIN_EPOCHS, BATCH_SIZE, LEARNING_RATE, WARMUP_RATIO
)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"FINE-TUNING (v{version})")
    print(f"{'='*60}")

    # Check if required files exist
    for path, name in [(paths['train_split'], "Train split"), (paths['test_split'], "Test split"), (paths['products'], "Products")]:
        if not path.exists():
    ...
    print(f"  Product: '{train_data[0]['positive'][:100]}...'")
    ...
    # Queries and relevant docs from test split
    queries = {}
    relevant_docs = {}
    ...
    prefix = "amazon-products-finetune_"
    ...
    # Save the fine-tuned model to versioned directory
